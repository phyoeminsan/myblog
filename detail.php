<?php
    ...
    $post_id = $_GET['id'];
    // echo $post_id;
    $sql = "SELECT posts.*,categories.name as category_name,users.name as user_name FROM posts INNER JOIN categories ON category_id = categories.id INNER JOIN users ON user_id = users.id WHERE posts.id=:postID";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':postID',$post_id); //user binParam very secure
    $stmt->execute();
    $post = $stmt->fetch();
?>
<!-- Page content-->
